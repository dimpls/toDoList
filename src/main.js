...
class TodoService {
    ...
    updateTodo(number, title, description) {
        ...
        this.toDoList.replaceChild(this._createTodo(number, title, description), this.toDoList.children.item(number - 1))
    }
    ...
    _handleUpdate(event) {
        const card = event.target.parentElement.parentElement;
        const modal = new ModalService(this, this.api, 'update', card.cardId);
        modal.open();
    }
}
...
class ModalService {
    ...
    _onUpdate(e) {
        ...
        this.api.update(this.card, formData)
        this.todoService.updateTodo(this.card, formData.title, formData.description);
